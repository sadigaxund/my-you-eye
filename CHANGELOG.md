...
- **`Terminal` fixed-height scrolling, colour schemes and window chrome, plus per-entry prompt overrides** — the box keeps a measured `rows` height and content scrolls up as lines land, instead of the window growing. `user`/`host`/`cwd`/`promptGlyph` can be set per entry and carry forward to subsequent lines, so a `cd` mid-session changes the prompt exactly as a real shell would.
